...
`,U={domain:"crazycauldron.art",cookMint:"",showMint:!1,treasuryWallet:"",minHold:2e3,social:{x:"none",telegram:"none"},shopEnabled:!1};async function ve(e,t=6e3){const a=new AbortController,n=setTimeout(()=>a.abort(),t);try{const o=await fetch(`${fe.httpUrl}${e}`,{signal:a.signal});return o.ok?await o.json():null}catch{return null}finally{clearTimeout(n)}}async function Ie(){const e=await ve("/public/config");return e?{...U,...e,social:{...U.social,...e.social??{}}}:U}async function Fe(){return ve("/stats",4e3)}function D(e){const t=e.trim();return t===""||t.toLowerCase()==="none"?null:t}function ze(e,t=6){return e.length<=t*2+3?e:`${e.slice(0,t)}…${e.slice(-t)}`}const Ne=[{id:"how-it-works",label:"how it works"},{id:"the-world",label:"the world"},{id:"skills",label:"skills"},{id:"cooking",label:"cooking"},{id:"wardrobe",label:"wardrobe"},{id:"roadmap",label:"roadmap"},{id:"token",label:"$COOK"},{id:"proof",label:"proof"}];function ee(e){const t=e?"":"/";return r`
  ...
  `}function Ke(){if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;const e=Array.from(document.querySelectorAll(".band-art"));if(e.length===0)return;let t=!1;const a=()=>{t=!1;const o=window.innerHeight/2;for(const l of e){const h=l.parentElement;if(!h)continue;const d=h.getBoundingClientRect();if(d.bottom<-200||d.top>window.innerHeight+200)continue;const c=d.top+d.height/2,b=Math.max(-60,Math.min(60,(c-o)/o*-48));l.style.transform=`translate3d(0, ${b.toFixed(1)}px, 0)`}},n=()=>{t||(t=!0,requestAnimationFrame(a))};window.addEventListener("scroll",n,{passive:!0}),window.addEventListener("resize",n,{passive:!0}),a()}function He(e){for(const t of Array.from(e.querySelectorAll("[data-copy]")))t.addEventListener("click",()=>{var o;const a=t.dataset.copy??"",n=t.textContent??"Copy";(o=navigator.clipboard)==null||o.writeText(a).then(()=>{t.textContent="Copied",setTimeout(()=>t.textContent=n,1600)}).catch(()=>{t.textContent="Press ⌘C",setTimeout(()=>t.textContent=n,2200)})})}const We=s.skills.maxLevel;function qe(e,t){const a=t[e],n=new Set(s.unlocks[e].map(l=>l.level));let o=0;for(let l=2;l<=a;l+=1)n.has(l)||(o+=s.skills.coreStat[e].perLevelPct);return o}function ke(e,t){const{windowPctAtLevel1:a,windowPctAtLevel20:n}=s.cooking,o=We-1,l=Math.min(Math.max(e.firecraft-1,0),o)/o,h=a+(n-a)*l,d=s.economy.pan.find(m=>m.tier===t),c=h+((d==null?void 0:d.windowBonusPct)??0),b=1+qe("knifework",e)/100;return Math.min(c*b,100)}const ae=45,ne=90;function $e(e,t){const a=Math.min(Math.max(e,0),ae),n=Math.min(Math.max(e*s.cooking.fineWindowMultiplier,a),ne),o=a/200,l=n/200,h=d=>Math.min(Math.max(d,0),1);return{centre:t,superbPct:a,finePct:n,superbFrom:h(t-o),superbTo:h(t+o),fineFrom:h(t-l),fineTo:h(t+l)}}const Be=[{id:"bronze",minHold:5e3},{id:"silver",minHold:2e4},{id:"gold",minHold:1e5}],je=[{id:"hat_01_chef",kind:"hat",name:"Chef toque",unlock:{type:"cookQuality",minQuality:"fine"}},{id:"hat_02_straw",kind:"hat",name:"Straw hat",unlock:{type:"skillLevel",skill:"foraging",level:10}},{id:"hat_03_mushroom",kind:"hat",name:"Mushroom cap",unlock:{type:"dishesCooked",count:50}},{id:"hat_04_acorn",kind:"hat",name:"Acorn cap",unlock:{type:"sectionRecipes",section:2}},{id:"hat_05_circlet",kind:"hat",name:"Circlet",unlock:{type:"anySkillLevel",level:20}},{id:"hat_06_dragonscale",kind:"hat",name:"Dragonscale hat",unlock:{type:"recipeQuality",recipeNumber:20,quality:"superb"}},{id:"hat_07_moonpetal",kind:"hat",name:"Moonpetal crown",unlock:{type:"recipeQuality",recipeNumber:19,quality:"superb"}},{id:"hat_08_bronze",kind:"hat",name:"Bronze toque",unlock:{type:"tier",tier:"bronze"}},{id:"hat_09_gold",kind:"hat",name:"Gold toque",unlock:{type:"tier",tier:"gold"}},{id:"companion_01_hen",kind:"companion",name:"Hen",unlock:{type:"start"}},{id:"companion_02_piglet",kind:"companion",name:"Piglet",unlock:{type:"chefLevel",level:5}},{id:"companion_03_berrymouse",kind:"companion",name:"Berrymouse",unlock:{type:"chefLevel",level:10}},{id:"companion_04_mole",kind:"companion",name:"Mole",unlock:{type:"skillLevel",skill:"prospecting",level:10}},{id:"companion_05_owlet",kind:"companion",name:"Owlet",unlock:{type:"chefLevel",level:20}},{id:"companion_06_glowslime",kind:"companion",name:"Glowslime",unlock:{type:"tier",tier:"silver"}},{id:"companion_07_emberfox",kind:"companion",name:"Emberfox",unlock:{type:"tier",tier:"gold"}}],De={tiers:Be,items:je},xe=De,z=xe.items,oe=xe.tiers;function Ge(e){var t;return((t=oe.find(a=>a.id===e))==null?void 0:t.minHold)??0}function Ce(e){switch(e.type){case"start":return"Yours from the start";case"chefLevel":return`Reach Chef Level ${e.level}`;case"skillLevel":return`Reach ${e.skill} ${e.level}`;case"anySkillLevel":return`Take any skill to ${e.level}`;case"dishesCooked":return`Cook ${e.count} dishes`;case"cookQuality":return`Cook any dish at ${e.minQuality} or better`;case"sectionRecipes":return"Cook every Deep Forest recipe at least once";case"recipeQuality":{const t=N[e.recipeNumber-1];return`Cook ${(t==null?void 0:t.name)??`recipe ${e.recipeNumber}`} at ${e.quality}`}case"tier":return`Hold ${Ge(e.tier).toLocaleString()} $COOK`;default:return"Locked"}}const C="/assets/generated",f={painting(e,t=!1){return`/assets/site/${e}-${t?"narrow":"wide"}.jpg`},paintingSrcset(e){return`${f.painting(e,!0)} 720w, ${f.painting(e,!1)} 1440w`},hat:e=>`${C}/hats/${e}.png`,companion:e=>`${C}/companions/${e}.png`,dish:e=>`${C}/dishes/${e}.png`,ingredient:e=>`${C}/ingredients/${e}.png`,prop:e=>`${C}/props/${e}.png`,node:e=>`${C}/nodes/${e}.png`,character:e=>`${C}/characters/${e}.png`,async hatOffsets(){var h,d,c;const e=async b=>{try{const m=await fetch(`${C}/${b}`);return m.ok?await m.json():null}catch{return null}},[t,a,n]=await Promise.all([e("offsets.default.json"),e("offsets.overrides.json"),e("manifest.json")]),o=((c=(d=(h=n==null?void 0:n.bodies)==null?void 0:h.male)==null?void 0:d.anchors)==null?void 0:c.headTop)??0,l={};for(const[b,m]of Object.entries((t==null?void 0:t.hats)??{}))m.down&&(l[b]=m.down);for(const[b,m]of Object.entries((a==null?void 0:a.hats)??{}))m.down&&(l[b]={x:m.down.x,y:m.down.y+o});return l}};function Xe(e){return new Promise(t=>{const a=new Image;a.decoding="async",a.onload=()=>t(a),a.onerror=()=>t(null),a.src=e})}async function me(e){var c,b,m,$;const[t,a,n]=await Promise.all([Xe(f.character(e)),fetch(`${C}/characters/${e}.json`).then(y=>y.ok?y.json():null).catch(()=>null),fetch(`${C}/manifest.json`).then(y=>y.ok?y.json():null).catch(()=>null)]);if(!t||!a)return null;const o=new Map(a.frames.map(y=>[y.filename,y.frame])),h=((($=(m=(b=(c=n==null?void 0:n.bodies)==null?void 0:c[e])==null?void 0:b.walk)==null?void 0:m.right)==null?void 0:$.order)??[0,1,2,3]).map(y=>o.get(`${e}_walk_right_${y}`)).filter(y=>y!==void 0),d=h[0]??o.get(`${e}_idle_down`);return!d||h.length===0?null:{sheet:t,walkRight:h,frameWidth:d.w,frameHeight:d.h}}const Ue={1:"map_meadows",2:"map_forest",3:"map_caves"};function Qe(e){const t=F.filter(a=>a.section===e);return r`
    <div class="grid" style="grid-template-columns: repeat(4, 1fr); gap: 0.35rem">
      ${t.map(a=>r`
          <div class="sprite-tile" title="${a.name}">
            <img
              class="sprite"
              src="${f.ingredient(a.id)}"
              alt="${a.name}"
              width="64"
              height="64"
              loading="lazy"
  ...
  `}function it(){const e=document.getElementById("heat-track"),t=document.getElementById("heat-fine"),a=document.getElementById("heat-superb"),n=document.getElementById("heat-marker"),o=document.getElementById("heat-result"),l=document.getElementById("heat-demo");if(!e||!t||!a||!n||!o||!l)return;let h=1,d=G(h),c=!0,b=!0,m=0;const $=()=>{t.style.left=`${d.fineFrom*100}%`,t.style.width=`${(d.fineTo-d.fineFrom)*100}%`,a.style.left=`${d.superbFrom*100}%`,a.style.width=`${(d.superbTo-d.superbFrom)*100}%`},y=()=>m>=d.superbFrom&&m<=d.superbTo?"Superb":m>=d.fineFrom&&m<=d.fineTo?"Fine":"Common";for(const x of Array.from(l.querySelectorAll("[data-firecraft]")))x.addEventListener("click",()=>{h=Number(x.dataset.firecraft??"1"),d=G(h);for(const O of Array.from(l.querySelectorAll("[data-firecraft]")))O.setAttribute("aria-pressed",String(O===x));c=!0,o.textContent=`Superb is ${d.superbPct.toFixed(1)}% of the bar here.`,$()});const T=l.querySelector("[data-heat-stop]"),R=()=>{c=!c,o.textContent=c?"A demo. Nothing here is scored or saved.":`${y()}. A demo — nothing here is scored or saved.`,T==null||T.setAttribute("aria-pressed",String(!c))};T==null||T.addEventListener("click",R),e.addEventListener("click",R);const I=s.cooking.barMs/s.cooking.markerSpeedMultiplier;let K=performance.now(),H=0;const W=x=>{const O=Math.min(64,x-K);if(K=x,c&&b){H+=O;const _=H%(I*2)/I;m=_<=1?_:2-_,n.style.left=`${m*100}%`}requestAnimationFrame(W)};"IntersectionObserver"in window&&new IntersectionObserver(x=>{for(const O of x)b=O.isIntersecting},{rootMargin:"120px"}).observe(e),$(),requestAnimationFrame(W)}function st(e){var t;return((t=M.find(a=>a.index===e))==null?void 0:t.name)??""}const lt={raw:"Raw",pan_fry:"Pan-fry",simmer:"Simmer",bake:"Bake",clay_bake:"Clay bake",roast:"Roast",smoke:"Smoke",chill:"Chill"};function dt(e){const t=Object.entries(e.requirements).map(([a,n])=>`${s.skills.names[a]} ${n}`);return t.length===0?"No requirements":t.join(" · ")}function ct(e){return r`
  ...
  `}function Tt(){return E({id:"roadmap",painting:"map_forest",label:"roadmap.txt",body:re()})}function St(e){return r`
    ...
    <main class="site-wrap">
      <section class="band" id="roadmap">
        <img
          class="band-art"
          src="${f.painting("map_forest",!1)}"
          srcset="${f.paintingSrcset("map_forest")}"
          sizes="100vw"
          alt=""
          aria-hidden="true"
          decoding="async"
  ...
  `}function Et(e,t){return r`
    ...
    ${Re(t)} ${te(e)}
  `}function Lt(e){for(const t of Array.from(document.querySelectorAll("[data-live]"))){const a=t.dataset.live;a&&typeof e[a]=="number"&&(t.textContent=A(e[a]))}}const Q=8e3,S=3;function J(e){return new Promise(t=>{const a=new Image;a.decoding="async",a.onload=()=>t(a),a.onerror=()=>t(null),a.src=e})}function Mt(e){const t=e.getContext("2d",{alpha:!0});if(!t)return()=>{};const a=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let n=0,o=0,l=null,h=[];const d=[];let c=null,b=performance.now()+4e3;const m=[];let $=0,y=!1;function T(){const p=e.getBoundingClientRect(),i=Math.min(window.devicePixelRatio||1,2);n=Math.max(1,Math.round(p.width)),o=Math.max(1,Math.round(p.height)),e.width=Math.round(n*i),e.height=Math.round(o*i),t.setTransform(i,0,0,i,0,0),t.imageSmoothingEnabled=!1,R(),I()}function R(){const p=a?0:Math.min(46,Math.round(n/26));h=Array.from({length:p},(i,u)=>{const g=(u+.5)/p*n+(Math.sin(u*12.9898)*.5+.5)*14,v=Math.sin(u*78.233)*.5+.5;return{x:g,y:o-6-v*Math.min(70,o*.1),height:7+v*9,phase:u*.7,shade:v>.5?"rgba(92, 138, 74, 0.55)":"rgba(63, 107, 70, 0.5)"}})}function I(){const p=o-Math.min(48,o*.07);for(let i=0;i<m.length;i+=1)m[i].y=p-i*6}function K(p){if(!l){t.fillStyle="#14101a",t.fillRect(0,0,n,o);return}const i=Math.max(n/l.width,o/l.height),u=l.width*i,g=l.height*i,v=a?0:ce(p,0,Q*3,n)*9;t.drawImage(l,(n-u)/2+v,(o-g)*.62,u,g)}function H(p){for(const i of h){const u=Oe(p,i.x,i.phase,Q,n);t.save(),t.translate(i.x,i.y),t.rotate(u.angle*Math.PI/180),t.fillStyle=i.shade,t.fillRect(-1,-i.height,2,i.height),t.fillRect(-3,-i.height*.6,2,i.height*.6),t.fillRect(1,-i.height*.75,2,i.height*.75),t.restore()}}function W(p,i){if(!a){if($%22===0)for(const u of[.26,.68])d.push({x:n*u,y:o*.34,age:0,life:5200,drift:0,size:5});for(let u=d.length-1;u>=0;u-=1){const g=d[u];if(g.age+=i,g.age>g.life){d.splice(u,1);continue}const v=g.age/g.life;g.drift+=ce(p,g.x,Q,n)*.35,t.globalAlpha=(1-v)*.22,t.fillStyle="#cfc6bb",t.beginPath(),t.arc(g.x+g.drift,g.y-v*o*.22,g.size+v*16,0,Math.PI*2),t.fill(),t.globalAlpha=1}}}function x(p,i){if(a||(!c&&p>b&&(c={x:-30,y:o*(.18+Math.random()*.14),speed:.045+Math.random()*.02,phase:Math.random()*Math.PI*2}),!c))return;if(c.x+=c.speed*i,c.x>n+30){c=null,b=p+9e3+Math.random()*9e3;return}const u=Math.sin(p/90+c.phase)*4;t.strokeStyle="rgba(24, 20, 30, 0.55)",t.lineWidth=2,t.beginPath(),t.moveTo(c.x-6,c.y+u),t.lineTo(c.x,c.y),t.lineTo(c.x+6,c.y+u),t.stroke()}function O(p){for(const i of m){i.x+=i.speed*p,i.x>n+60&&(i.x=-60-Math.random()*120),i.step+=p;const u=i.body.walkRight,g=u[Math.floor(i.step/100)%u.length],v=g.w*S,q=g.h*S,B=Math.round(i.x),w=Math.round(i.y-q),P=Math.floor(i.step/200)%2===0?0:S;if(t.drawImage(i.body.sheet,g.x,g.y,g.w,g.h,B,w+P,v,q),i.hat&&t.drawImage(i.hat,B+i.hatOffset.x*S,w+P+i.hatOffset.y*S,i.hat.width*S,i.hat.height*S),i.companion){const le=i.companion.width*S,de=i.companion.height*S;t.drawImage(i.companion,Math.round(i.x-le-8),Math.round(i.y-de+P),le,de)}}}let _=performance.now();function ie(p){if(y)return;const i=Math.min(64,p-_);_=p,$+=1,t.clearRect(0,0,n,o),K(p),W(p,i),x(p,i),H(p),O(i),requestAnimationFrame(ie)}const se=()=>T();return window.addEventListener("resize",se,{passive:!0}),T(),requestAnimationFrame(ie),(async()=>{const p=window.innerWidth<900;l=await J(f.painting("map_hub",p));const i=await Promise.all([me("male"),me("female")]),u=["hat_01_chef","hat_05_circlet","hat_03_mushroom","hat_07_moonpetal"],g=["companion_01_hen","companion_07_emberfox","companion_02_piglet",null],[v,q,B]=await Promise.all([Promise.all(u.map(w=>J(f.hat(w)))),Promise.all(g.map(w=>w?J(f.companion(w)):Promise.resolve(null))),f.hatOffsets()]);for(let w=0;w<u.length;w+=1){const P=i[w%2];P&&m.push({x:-80-w*150,y:o,speed:.022+w*.004,body:P,hat:v[w]??null,hatOffset:B[u[w]??""]??{x:6,y:-12},companion:q[w]??null,step:w*340})}I()})(),()=>{y=!0,window.removeEventListener("resize",se)}}const Y=e=>Math.round(e/6e4);function V(e,t=1,a=0){return $e(ke({foraging:1,prospecting:1,knifework:t,firecraft:e,spicecraft:1},a),.5)}const _t={raw:"Raw",pan_fry:"Pan-fry",simmer:"Simmer",bake:"Bake",clay_bake:"Clay bake",roast:"Roast",smoke:"Smoke",chill:"Chill"};function ue(e){var t;return((t=M.find(a=>a.index===e))==null?void 0:t.name)??""}function Pt(e){return[{id:"what-it-is",title:"1. What CrazyCauldron is",body:()=>r`
        <p>
                ...
                ${N.map(o=>r`
                    <tr>
                      ...
                      <td class="small muted">
                        ${Object.entries(o.requirements).map(([l,h])=>`${s.skills.names[l]} ${h}`).join(", ")||"—"}
                      </td>
                    </tr>
                  `)}
              </tbody>
